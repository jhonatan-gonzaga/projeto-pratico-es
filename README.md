# Projeto Prático I - Engenharia de Software I

## Visão Geral do Projeto: Conecta Obra Itacoatiara

O projeto **Conecta Obra Itacoatiara** visa criar uma plataforma para conectar profissionais autônomos da construção civil na cidade de Itacoatiara, Amazonas, com clientes que necessitam de seus serviços. A iniciativa busca resolver a dificuldade de profissionais em encontrar trabalho consistente e a dependência de indicações "boca a boca", promovendo um canal digital para mais oportunidades e estabilidade.

## Documentação do Projeto

Os seguintes documentos e quadros foram elaborados como parte da especificação e organização do projeto:

*   **Plano de Trabalho:** Detalha a organização e o cronograma das atividades do projeto.
    *   [1_plano-de-trabalho.md](especificacao/1_plano-de-trabalho.md)

*   **Design Thinking:** Apresenta as fases de Inspiração e Ideação, incluindo Golden Circle, Análise SWOT, Identificação Visual de Soluções Existentes, Quadro Comparativo e Personas.
    *   [2_design-thinking.md](especificacao/2_design-thinking.md)
    *   [Fase de Ideação (Resultado Final)](IDEACAO.md)
    *   [Página de Ideação no Notion](https://www.notion.so/33a60f8e67b3811198b7d8cd2fd17e5c)

*   **Backlog do Produto:** Lista priorizada de funcionalidades desejadas para o sistema, escritas na forma de histórias de usuário, guiando o desenvolvimento incremental.
    *   [3_backlog-do-produto.md](especificacao/3_backlog-do-produto.md)
    *   [Quadro de Backlog do Produto - GitHub Projects](https://github.com/users/jhonatan-gonzaga/projects/3)

*   **Backlog do Sprint:** Detalha o conjunto de itens do Backlog do Produto selecionados para serem entregues durante a sprint atual, juntamente com o plano para transformá-los em um incremento de produto funcional.
    *   [4_backlog-do-sprint.md](especificacao/4_backlog-do-sprint.md)
    *   [Quadro de Backlog do Sprint - GitHub Projects](https://github.com/users/jhonatan-gonzaga/projects/3)

## Acompanhamento Diário (Dailys)

O registro das atividades diárias da equipe é realizado no Notion, onde cada integrante reporta o que fez, o que fará e eventuais impedimentos. A coluna "numero da tarefa" no Notion está diretamente associada às tarefas do Backlog do Sprint no GitHub Projects, garantindo a rastreabilidade entre o relato diário e a execução técnica.

*   [Daily Scrum - Notion](https://www.notion.so/4b4f2e04b56a48aa895c4082f9f65424?v=039de6463eb8436abf9746d426406901)

---

**Equipe:**

*   Jhonatan Gonzaga
*   Victor dos Santos
*   Rodrigo Torres
*   Alyce Benevides
*   Oliviê Kalil

**Professor:** Prof. Dr. Andrey Rodrigues

**Disciplina:** Engenharia de Software I

**Instituição:** Universidade Federal do Amazonas (UFAM)
